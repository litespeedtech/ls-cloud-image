...
.
csrconf
        certbot certonly --non-interactive --agree-tos --email $w_email --webroot -w $w_webroot -d $DOMAIN --csr ecc.csr >> $LOG_FILE 2>&1
    elif [ ${TYPE} = 2 ]; then
        openssl ecparam -genkey -name secp384r1 | sudo openssl ec -out ecc.key >> $LOG_FILE 2>&1
        ...
        certbot certonly --non-interactive --agree-tos --email $w_email --webroot -w $w_webroot -d $DOMAIN -d $WWW_DOMAIN  --csr ecc.csr >> $LOG_FILE 2>&1
    else
