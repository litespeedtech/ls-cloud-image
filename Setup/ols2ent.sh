...
echo -e "Please input the backup directory :\n"
...
install_lsws() {
#wget -O functions.sh https://raw.githubusercontent.com/usmannasir/cyberpanel/stable/install/litespeed/functions.sh
#wget -O install.sh https://raw.githubusercontent.com/usmannasir/cyberpanel/stable/install/litespeed/install.sh
#chmod +x functions.sh

if [[ $pkg_tool == "apt" ]] ; then
  #sed -i $'s/TMP_GROUP=\'nobody\'/TMP_GROUP=\'nogroup\'/' functions.sh
  #sed -i $'s/TMP_GROUP=\'nobody\'/TMP_GROUP=\'www-data\'/' functions.sh
  #sed -i $'s/TMP_USER=\'nobody\'/TMP_USER=\'www-data\'/' functions.sh
  USER="www-data"
  GROUP="www-data"
else
  USER="nobody"
...
wget -O /usr/local/lsws/conf/httpd_config.xml https://cyberpanel.sh/misc/httpd_config.xml
if [[ $pkg_tool == "apt" ]] ; then
...
wget -q -O /root/.litespeed_conf/httpd.conf https://cyberpanel.sh/misc/httpd.conf
fi
...
wget -q -O /root/.litespeed_conf/conf/vhosts/${domains[i]}/${domains[i]}.conf https://cyberpanel.sh/misc/example.conf
...
sed -i 's|replacement_IP|'$server_ip'|g' /root/.litespeed_conf/httpd.conf
