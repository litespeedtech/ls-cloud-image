...
CKREG="^[A-Za-z0-9.!#$%&'*+/=?^_\`{|}~-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)+$"
THEME='twentytwenty'
PHPVER=lsphp83
WEBSERVER='OLS'
USER='www-data'
...
show_help() {
    case ${1} in
        "1")
            ...
            echo "${EPACE}${EPACE}Issue Let's Encrypt certificate, must follow with E-mail address."
            echow "-F, --force-https"
...
check_provider() {
    if [[ -r /sys/devices/virtual/dmi/id/product_uuid ]] && [[ "$(cat /sys/devices/virtual/dmi/id/product_uuid | cut -c 1-3)" =~ (EC2|ec2) ]]; then
        PROVIDER='aws'
    elif [ "$(dmidecode -s bios-vendor 2>/dev/null)" = 'Google' ]; then
        PROVIDER='google'
    elif [ "$(dmidecode -s system-product-name 2>/dev/null | cut -c 1-7)" = 'Alibaba' ]; then
        PROVIDER='aliyun'
    elif [ "$(dmidecode -s system-manufacturer 2>/dev/null)" = 'Microsoft Corporation' ]; then
        PROVIDER='azure'
...
check_php_version() {
    if [ -e "${LSDIR}/fcgi-bin/lsphpnew" ]; then
        ...
        PHP_MA="$(php -r 'echo PHP_MAJOR_VERSION;' 2>/dev/null)"
        PHP_MI="$(php -r 'echo PHP_MINOR_VERSION;' 2>/dev/null)"
    fi
...
line_insert() {
    ...
    MATCHES=$(grep -n "${PATTERN}" "${FILE}" | cut -d: -f1 | sort -nr)

    if [ -n "${MATCHES}" ]; then
        for LINENUM in ${MATCHES}; do
            if [ -n "${LINENUM}" ] && [ "${LINENUM}" -eq "${LINENUM}" ] 2>/dev/null; then
                LINENUM=$((${LINENUM}+${ADDNUM}))
...
gen_password() {
    if [ -e "${HM_PATH}/.db_password" ]; then
        ROOT_PASS=$(head -n 1 "${HM_PATH}/.db_password" | awk -F '"' '{print $2}')
    fi
    ...
    if [ -z "${USERPASSWORD}" ]; then
        USERPASSWORD=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 48)
    fi
}
...
EOF
        ...
        chown -R lsadm:lsadm "${VHDIR}/${MY_DOMAIN}"
    else
...
set_ols_server_conf() {
    ...
    if [ ${WWW} = 'TRUE' ]; then
        ...
        TEMP_DOMAIN=${MY_DOMAIN2}
    else
        NEWKEY="map                     ${MY_DOMAIN} ${MY_DOMAIN}"
        TEMP_DOMAIN=${MY_DOMAIN}
    fi

    LISTENER_LINES=$(grep -nE "^[[:space:]]*address[[:space:]]+.*:[0-9]+$" "${WEBCF}" | cut -d: -f1 | sort -nr)

    if [ -n "${LISTENER_LINES}" ]; then
        for LINENUM in ${LISTENER_LINES}; do
            sed -i "$((LINENUM + 2))i${NEWKEY}" "${WEBCF}"
        done
    else
...
verify_domain() {
    ...
    if [ ${WWW} = 'TRUE' ]; then
        curl -Is "http://${MY_DOMAIN2}/" | grep -i 'LiteSpeed|cloudflare' >/dev/null 2>&1
        if [ $? = 0 ]; then
...
EOF
            cat "${DOCHM}/.htaccess" >> "${TMP_HT}"
            cat "${TMP_HT}" > "${DOCHM}/.htaccess"
            rm -f "${TMP_HT}"
            restart_lsws
